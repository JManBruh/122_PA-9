...
/*
FIXES BY TAYLOR:
Removed window.clear(), window.display(), and event polling from draw(); initialized arraySize to 0.
added pragma once
*/
...
void batch::setOrg(Vector2f in, int num) { origins[num] = in; }
void batch::setPos(Vector2f in, int num) { positions[num] = in; }
void batch::setRot(Angle in, int num) { angles[num] = in; }
